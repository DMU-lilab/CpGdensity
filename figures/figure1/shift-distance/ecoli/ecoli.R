...
pdf(paste(specie,"-",target.pattern,"-distance-figure.pdf",sep=""))
theme_set(theme_bw(12))
ggplot(tp.shif,aes(sh20-pos))+geom_histogram(binwidth=1)+xlim(0,600)+labs(title=paste(specie,"cg distance shift number = ",strsplit(names(tp.shif)[6],"h")[[1]][2]))
ggplot(tp.shif)+geom_density(aes(sh20-pos),fill="RoyalBlue")+xlim(0,600) +labs(title=paste(specie,"cg distance shift number = ",strsplit(names(tp.shif)[6],"h")[[1]][2]))
dev.off()
